...
def test_decode():
    ...
    encoder.undo()
    encoder.increment()
    encoder.encode(text[len(image)-10])
    result = encoder.decode()
    introcs.assert_equals(text[len(image)-10],result)
...
def test_all():
    ...
    test_jail()
    test_vignette()
    #test_pixellate()         # Optional method
    print('Class Filter passed all tests.')
    print()

    print('Testing class Encoder')
    test_encode()
    test_decode()
    print('Class Encoder passed all tests.')
